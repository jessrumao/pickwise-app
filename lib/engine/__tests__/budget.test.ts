...
describe("budget allocator, end to end on vegetarian-muscle-gain (budget: 3000 INR, hard)", () => {
  it("funds in priority order (never cheapest-first) until the budget runs out, defers the rest with price shown", () => {
    ...
    // both cheaper and better value. Naive total (1799 + 799 + 789 + 1200 +
    // 525 = 5112) still exceeds the 3000 budget, and none of
    // magnesium/omega-3(algal-oil)/multivitamin has a cheaper alternative SKU
    // in the catalogue today, so all three still defer rather than bumping
    // cheaper protein/creatine out of priority order.
